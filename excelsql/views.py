...
print(CUR_DIR)
def home(request):
    return render(request, 'excelsql/base.html')

#p.save_book_as(file_name=CUR_DIR + '/balance.xls',
                          #dest_file_name=CUR_DIR + '/balance.xlsx')
...
def export_to_sqlite(request):
    ...
    # Имя базы
    base_name = os.path.join(BASE_DIR, 'db.sqlite3')
    ...
    # закрытие соединения
    connect.close()
    return render(request, 'excelsql/import_result.html', context={'link': os.path.join(BASE_DIR, 'db.sqlite3')})
